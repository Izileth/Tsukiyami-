...
export const PostsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getPostById = useCallback((id: number) => posts.find((post) => post.id === id), [posts]);

  const incrementView = useCallback(async (postId: number) => {
    ...
    const { error } = await supabase
      .from('post_views')
      .insert({ post_id: postId, user_id: user.id }, { onConflict: 'post_id, user_id' });

    if (!error) {
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, views_count: p.views_count + 1 } : p));
      setViewedPosts(prev => new Set(prev).add(postId));
    } else if (error.code !== '23505') { // 23505 is unique_violation, which we expect
      console.error('Error incrementing view:', error);
    }
  }, [viewedPosts]);

  const updatePostLikes = useCallback((postId: number, newLikesCount: number) => {
    setPosts(prev => prev.map(p => p.id === postId ? { ...p, likes_count: newLikesCount } : p));
  }, []);

  const updatePostDislikes = useCallback((postId: number, newDislikesCount: number) => {
    setPosts(prev => prev.map(p => p.id === postId ? { ...p, dislikes_count: newDislikesCount } : p));
  }, []);

  const updatePostCommentsCount = useCallback((postId: number, newCommentsCount: number) => {
    setPosts(prev => prev.map(p => p.id === postId ? { ...p, comments_count: newCommentsCount } : p));
  }, []);
